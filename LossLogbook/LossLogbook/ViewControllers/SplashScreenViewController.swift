...
class SplashScreenViewController: UIViewController {
    ...
    private(set) lazy var animationDidFinishSubject = PublishSubject<Void>()
    var rx_animationDidFinish: Observable<Void> {
        return animationDidFinishSubject.asObservable()
    }
        ...
        Model.shared.loadData { state in
           if state {
               DispatchQueue.main.async { [weak self] in
                   ...
                   self?.animationDidFinishSubject.onNext(())
...
extension SplashScreenViewController: SplashScreenViewControllerDelegate {
    func splashScreenAnimationDidFinish() {
        delegate?.splashScreenAnimationDidFinish()
    }
}
